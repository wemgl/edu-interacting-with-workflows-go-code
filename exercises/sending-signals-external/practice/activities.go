...
func MakePizzas(ctx context.Context, order PizzaOrder) error {
	logger := activity.GetLogger(ctx)
	logger.Info("Starting to make pizzas for order", order.OrderNumber)

	for _, pizza := range order.Items {
		logger.Info("Making pizza:", pizza.Description)
		// Simulate the time taken to make a pizza.
	}

	logger.Info("All pizzas for order", order.OrderNumber, "are ready!")
	return nil
}

func DeliverPizzas(ctx context.Context, order PizzaOrder) error {
	logger := activity.GetLogger(ctx)
	logger.Info("Starting delivery", order.OrderNumber, "to", order.Address)
	// Simulate the time to make delivery
	logger.Info(order.OrderNumber, "delivered.")
	return nil
}
